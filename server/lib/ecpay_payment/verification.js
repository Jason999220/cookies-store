...
class PaymentVerifyBase {
  constructor() {
    this.param_xml_file = fs
      .readFileSync(__dirname + "/../../lib/ecpay_payment/ECpayPayment.xml")
      .toString();
    this.param_xml = et.parse(this.param_xml_file);
  }

  get_svc_url(apiname, mode) {
    let url = this.param_xml.findtext(
      `./${apiname}/ServiceAddress/url[@type=\"${mode}\"]`
    );
    return url;
  }
  ...
  get_special_encode_param(apiname) {
    let ret = [];
    let node = this.param_xml.findall(
      `./${apiname}/Parameters//param[@urlencode=\"1\"]`
    );
    // console.log(node);
  ...
  get_basic_params(apiname) {
    let basic_param = [];
    this.param_xml
      .findall(`./${apiname}/Parameters/param[@require=\"1\"]`)
  ...
  get_cond_param(apiname) {
    ...
    this.param_xml
      .findall(`./${apiname}/Config/switchparam/n`)
      .forEach(function (elem) {
    ...
    aio_sw_param.forEach(function (pname) {
      let opt_param = {};
      let node = param_xml.findall(
        `./${apiname}/Parameters//param[@name=\"${pname}\"]/condparam`
      );
      node.forEach(function (elem) {
        ...
        param_xml
          .findall(
            `./${apiname}/Parameters//param[@name=\"${pname}\"]//condparam[@owner=\"${opt}\"]/param[@require="1"]`
          )
          .forEach(function (pa) {
  ...
  get_param_type(apiname) {
    let param_type = {};
    this.param_xml
      .findall(`./${apiname}/Parameters//param`)
      .forEach(function (elem) {
  ...
  get_opt_param_pattern(apiname) {
    ...
    let param_xml = this.param_xml;
    let node = this.param_xml.findall(
      `./${apiname}/Parameters//param[@type=\"Opt\"]`
    );
    ...
    temp_arr.forEach(function (opt_params) {
      ...
      let opt_elems = param_xml.findall(
        `./${apiname}/Parameters//param[@name=\"${opt_params}\"]//option`
      );
  ...
  get_int_param_pattern(apiname) {
    ...
    let param_xml = this.param_xml;
    let node = this.param_xml.findall(
      `./${apiname}/Parameters//param[@type=\"Int\"]`
    );
    node.forEach(function (param_elem) {
      temp_arr.push(param_elem.attrib.name);
    });
    ...
    temp_arr.forEach(function (opt_params) {
      // console.log(opt_params);
      let mode = param_xml.findall(
        `./${apiname}/Parameters//param[@name=\"${opt_params}\"]//mode`
      );
      let mx = param_xml.findall(
        `./${apiname}/Parameters//param[@name=\"${opt_params}\"]//maximum`
      );
      let mn = param_xml.findall(
        `./${apiname}/Parameters//param[@name=\"${opt_params}\"]//minimal`
      );
      // console.log(mode);
      let arr = [];
  ...
  get_str_param_pattern(apiname) {
    ...
    let param_xml = this.param_xml;
    let node = this.param_xml.findall(
      `./${apiname}/Parameters//param[@type=\"String\"]`
    );
    node.forEach(function (param_elem) {
      temp_arr.push(param_elem.attrib.name);
    });
    ...
    temp_arr.forEach(function (opt_params) {
      // console.log(opt_params);
      let pat_elems = param_xml.findall(
        `./${apiname}/Parameters//param[@name=\"${opt_params}\"]//pattern`
      );
      let arr = [];
  ...
  get_depopt_param_pattern(apiname) {
    ...
    let parent_n_opts = {};
    let node = this.param_xml.findall(
      `./${apiname}/Parameters//param[@type=\"DepOpt\"]`
    );
    node.forEach(function (param_elem) {
      p_name = param_elem.attrib.name;
      parent_name = param_elem.attrib.main;
    });
    k = this.param_xml.findall(
      `./${apiname}/Parameters//param[@name=\"${p_name}\"]//mainoption`
    );
    k.forEach(function (elem) {
      k_opts.push(elem.attrib.name);
    });
    k_opts.forEach(function (elem) {
      get_opts = param_xml.findall(
        `./${apiname}/Parameters//mainoption[@name=\"${elem}\"]//option`
      );
      let opt = [];
    ...
    Object.keys(params).forEach(function (p_name) {
      ...
      switch (p_type) {
        case "String":
          ...
          if (mat === null) {
            throw new ECpayError.ECpayInvalidParam(
              `Wrong format of param ${p_name} or length exceeded.`
            );
          }
          break;
        case "Opt":
          ...
          if (mat_opt === false) {
            throw new ECpayError.ECpayInvalidParam(
              `Unexpected option of param ${p_name} (${params[p_name]}). Available option: (${aval_opt}).`
            );
          }
          break;
        case "Int":
          ...
          switch (mode) {
            case "BETWEEN":
              if (val < min || val > max) {
                throw new ECpayError.ECpayInvalidParam(
                  `Value of ${p_name} should be between ${min} and ${max}.`
                );
              }
              break;
            case "GE":
              if (val < min) {
                throw new ECpayError.ECpayInvalidParam(
                  `Value of ${p_name} should be greater than or equal to ${min}.`
                );
              }
              break;
            case "LE":
              if (val > max) {
                throw new ECpayError.ECpayInvalidParam(
                  `Value of ${p_name} should be less than or equal to ${max}.`
                );
              }
              break;
            case "EXCLUDE":
              if (val >= max && val <= max) {
                throw new ECpayError.ECpayInvalidParam(
                  `Value of ${p_name} can NOT be between ${min} and ${max}..`
                );
              }
              break;
            default:
              throw new ECpayError.ECpayInvalidParam(
                `Unexpected integer verification mode for parameter ${p_name}: ${mode}.`
              );
          }
          break;
        case "DepOpt":
          ...
          if (
            aval_dopt === null &&
            pattern["Opt"][parent_param].includes(parent_val) === false
          ) {
            throw new ECpayError.ECpayInvalidParam(
              `Cannot find available option of [${p_name}] by related param [${parent_param}](Value: ${parent_val}.`
            );
          } else if (aval_dopt.constructor === Array) {
            if (!aval_dopt.includes(params[p_name])) {
              throw new ECpayError.ECpayInvalidParam(
                `Unexpected option of param ${p_name} (${params[p_name]}). Available option: (${aval_dopt}).`
              );
            }
          }
          break;
        default:
          throw new Error(
            `Unexpected type (${p_type}) for parameter ${p_name}.`
          );
      }
    });
  }
  ...
  verify_aio_inv_param(params) {
    if (params.constructor === Object) {
      ...
      inv_param.forEach(function (pname) {
        if (input_param.indexOf(pname, 0) === -1) {
          throw new ECpayError.ECpayInvalidParam(
            `Lack required invoice param ${pname}`
          );
        }
      });
      input_param.forEach(function (pname) {
        if (inv_param.indexOf(pname, 0) === -1) {
          throw new ECpayError.ECpayInvalidParam(
            `Unexpected parameter in Invoice parameters ${pname}`
          );
        }
      });
      ...
      if (params["CarruerType"] === "1") {
        if (params["CustomerID"] === "") {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[CustomerID] can not be empty when [CarruryType] is 1.`
          );
        }
      ...
      if (params["Print"] === "1") {
        if (params["CustomerName"] === "" && params["CustomerAddr"] === "") {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[CustomerName] and [CustomerAddr] can not be empty when [Print] is 1.`
          );
        }
        if (params["CustomerID"] !== "") {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[Print] can not be '1' when [CustomerID] is not empty.`
          );
        }
      }
      ...
      if (params["CustomerPhone"] === "" && params["CustomerEmail"] === "") {
        throw new ECpayError.ECpayInvoiceRuleViolate(
          `[CustomerPhone] and [CustomerEmail] can not both be empty.`
        );
      }
      // d 別[TaxType]為2 => ClearanceMark = 1 or 2 and InvoiceItemTaxType must include '2'
      if (params["TaxType"] === "2") {
        if (
          params["ClearanceMark"] !== "1" &&
          params["ClearanceMark"] !== "2"
        ) {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[ClearanceMark] has to be 1 or 2 when [TaxType] is 2.`
          );
        }
        if (!params["InvoiceItemTaxType"].includes("2")) {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[InvoiceItemTaxType] must contain at lease one '2' when [TaxType] is 2.`
          );
        }
      }
      ...
      if (params["CustomerIdentifier"] !== "") {
        if (params["CarruerType"] === "1" || params["CarruerType"] === "2") {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[CarruerType] Cannot be 1 or 2 when [CustomerIdentifier] is given.`
          );
        }
      }
      ...
      if (params["CarruerType"] === "" || params["CarruerType"] === "1") {
        if (params["CarruerNum"] !== "") {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[CarruerNum] must be empty when [CarruerType] is empty or 1.`
          );
        }
      } else if (params["CarruerType"] === "2") {
        if (
          params["CarruerNum"].match(new RegExp(/[A-Za-z]{2}[0-9]{14}/)) ===
          null
        ) {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[CarruerNum] must be 2 alphabets and 14 numbers when [CarruerType] is 2.`
          );
        }
      } else if (params["CarruerType"] === "3") {
        if (
          params["CarruerNum"].match(new RegExp(/^\/[A-Za-z0-9\s+-]{7}$/)) ===
          null
        ) {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[CarruerNum] must start with '/' followed by 7 alphabet and number characters when [CarruerType] is 3.`
          );
        }
      } else {
        throw new ECpayError.ECpayInvoiceRuleViolate(
          `Unexpected value in [CarruerType].`
        );
      }
      // [CarruerType]有值時，Print必須有為0
      if (params["CarruerType"] !== "" && params["Print"] !== "0") {
        throw new ECpayError.ECpayInvoiceRuleViolate(
          `[Print] must be 0 when [CarruerType] has value.`
        );
      }
      ...
      if (params["Donation"] === "1") {
        if (params["LoveCode"] === "") {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[LoveCode] cannot be empty when [Donation] is 1.`
          );
        }
        if (params["Print"] !== "0") {
          throw new ECpayError.ECpayInvoiceRuleViolate(
            `[Print] must be 0 when [Donation] is 1.`
          );
        }
      }
      ...
      if (params["InvoiceItemName"] === "") {
        ...
        if (params["InvoiceItemName"].includes("|")) {
          ...
          item_params.forEach(function (param_name) {
            ...
            if (
              params[param_name].match(new RegExp(/(\|\||^\||\|$)/)) !== null
            ) {
              throw new ECpayError.ECpayInvoiceRuleViolate(
                `[${param_name}] contains empty value.`
              );
            }
            let p_cnt = params[param_name].split("|").length;
            if (item_cnt !== p_cnt) {
              throw new ECpayError.ECpayInvoiceRuleViolate(
                `Count of item info [${param_name}] (${p_cnt}) not match count from [InvoiceItemName] (${item_cnt}).`
              );
            }
          });
          ...
          if (vio_tax_t === []) {
            throw new ECpayError.ECpayInvoiceRuleViolate(
              `Illegal [InvoiceItemTaxType]: ${vio_tax_t}`
            );
          }
          if (params["TaxType"] === "9") {
            if (!item_tax.includes("1")) {
              throw new ECpayError.ECpayInvoiceRuleViolate(
                `[InvoiceItemTaxType] must contain at lease one '1'.`
              );
            }
            if (item_tax.includes("2") && item_tax.includes("3")) {
              throw new ECpayError.ECpayInvoiceRuleViolate(
                `[InvoiceItemTaxType] cannot contain 2 and 3 at the same time.`
              );
            }
          }
        } else {
          ...
          item_params.forEach(function (param_name) {
            if (params[param_name].includes("|")) {
              throw new ECpayError.ECpayInvoiceRuleViolate(
                `Item info [${param_name}] contain pipeline delimiter but there's only one item in param [InvoiceItemName].`
              );
            }
          });
        }
