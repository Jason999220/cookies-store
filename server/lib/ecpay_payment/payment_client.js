...
class ECpayPaymentClient {
    ...
    if (period_info.constructor === Object) {
      ...
      period_args.sort().forEach(function (pname) {
        if (Object.keys(period_info).sort().indexOf(pname, 0) === -1) {
          throw new Error(
            `Credit card period parameters must be ${period_args}.`
          );
        }
      });
    ...
    if (params.constructor === Object) {
      ...
      if (this.helper.is_contractor()) {
        ...
        if (params["MerchantID"] === null) {
          throw new Error(
            `[MerchantID] should be specified when you're contractor-Platform.`
          );
        }
      } else {
        params["PlatformID"] = "";
      ...
      throw new ECpayError.ECpayInvalidParam(
        `Received parameter object must be a Object.`
      );
    }
  }
  ...
  _aiochkout_pos_proc(params) {
    ...
    let sp_param =
      this.verify_aiochkout.get_special_encode_param("AioCheckOut");
    ...
    // gen post html
    let api_url = this.verify_aiochkout.get_svc_url(
      "AioCheckOut",
      this.helper.get_op_mode()
    );
